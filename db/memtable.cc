...
Status MemTable::SendToRemote(
    ...
    std::queue<std::pair<uint64_t, uint64_t>>* gc_queue, bool need_mark) {
  ...
  Status s =
      table_->SendToRemote(client, memtable_conn, remote_index_reg,
                           local_index_offset, (cfd_id << 32) | GetID(), 0);
  if (s.ok()) {
    // MarkAsFinished
...
static bool RemoteSaveValue(void* arg, const char* entry, void* rdma_buf) {
  ...
  // LOG_CERR("entry: ", entry);
  const char* key_ptr = GetVarint32Ptr(entry, entry + 5, &key_length);
  // LOG_CERR("key_ptr: ", key_ptr);
  // LOG_CERR("key_ptr address: ", static_cast<const void*>(key_ptr));
  // LOG_CERR("key_length", key_length);
  ...
  if (user_comparator->EqualWithoutTimestamp(user_key_slice,
    ...
    switch (type) {
      ...
      case kTypeValue: {
        // LOG_CERR("key_ptr + key_length: ", key_ptr + key_length);
        
        Slice v = GetLengthPrefixedSlice(key_ptr + key_length);
        // LOG_CERR("v.data(): ", v.data());
        // LOG_CERR("v.size(): ", v.size());
        if (s->status == nullptr) s->status = new Status();
        *(s->status) = Status::OK();
        if (s->value) {
          ...
          // LOG_CERR("get remote value:", *(s->value));
          // LOG_CERR("get remote value pointer:", v.data());
        }
