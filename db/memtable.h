...
class MemTable {
 ...
 public:
  ...
  bool OneSideGet(const LookupKey& key, std::string* value,
           PinnableWideColumns* columns, std::string* timestamp, Status* s,
           MergeContext* merge_context,
           SequenceNumber* max_covering_tombstone_seq, SequenceNumber* seq,
           const ReadOptions& read_opts, bool immutable_memtable,
  ...
  bool OneSideGet(const LookupKey& key, std::string* value,
           ...
           RDMANode::rdma_connection* conn = nullptr, uint64_t cfd_id = 0) {
    SequenceNumber seq;
    return OneSideGet(key, value, columns, timestamp, s, merge_context,
               max_covering_tombstone_seq, &seq, read_opts, immutable_memtable,
               callback, is_blob_index, do_merge, read_client, conn, cfd_id);
  }

  bool CacheGet(const LookupKey& key, std::string* value,
           PinnableWideColumns* columns, std::string* timestamp, Status* s,
           MergeContext* merge_context,
           SequenceNumber* max_covering_tombstone_seq, SequenceNumber* seq,
           const ReadOptions& read_opts, bool immutable_memtable,
           ReadCallback* callback = nullptr, bool* is_blob_index = nullptr,
           bool do_merge = true, RDMAReadClient* read_client = nullptr,
           RDMANode::rdma_connection* conn = nullptr, uint64_t cfd_id = 0, IndexCache* cache = nullptr, bool* bloom_contains = nullptr);
  ...
  bool CacheGet(const LookupKey& key, std::string* value,
           ...
           RDMANode::rdma_connection* conn = nullptr, uint64_t cfd_id = 0, IndexCache* cache = nullptr, bool* bloom_contains = nullptr) {
    SequenceNumber seq;
    return CacheGet(key, value, columns, timestamp, s, merge_context,
               max_covering_tombstone_seq, &seq, read_opts, immutable_memtable,
               callback, is_blob_index, do_merge, read_client, conn, cfd_id, cache, bloom_contains);
  }
  ...
  // MemTableRep* GetTable() {return table_;}

  uint64_t GetID() const { return id_; }

  const std::unordered_map<std::string, std::atomic<unsigned char>>* GetMapFreq() const {
    return &map_freq;
  }
 ...
 private:
  ...
  int64_t offset_ = 0;
  std::unordered_map<std::string, std::atomic<unsigned char>> map_freq;
  std::mutex map_mutex;
  ...
  void GetFromRemoteTable(const LookupKey& key,
                  SequenceNumber max_covering_tombstone_seq, bool do_merge,
                  ReadCallback* callback, bool* is_blob_index,
                  std::string* value, PinnableWideColumns* columns,
                  std::string* timestamp, Status* s,
  ...
  void GetFromCache(const LookupKey& key,
                SequenceNumber max_covering_tombstone_seq, bool do_merge,
                ReadCallback* callback, bool* is_blob_index,
                std::string* value, PinnableWideColumns* columns,
                std::string* timestamp, Status* s,
