...
bool MemTableListVersion::GetFromList(
    ...
    std::chrono::high_resolution_clock::time_point end_start =
      std::chrono::high_resolution_clock::now();
  bool need_remote_read = false;
  bool enable_multi_one_side_reads = false;
  std::vector<uint64_t> mixed_ids;
  *seq = kMaxSequenceNumber;
  // LOG_CERR("value pointer is: ", value);
  // if (list->size() > 0) {
  //   std::string now1;
  //   for (auto& id_ : *list) {
  ...
  for (auto& memtable : *list) {
    ...
    if (!need_remote_read &&
      ...
      if (!done && !s->ok() && !s->IsMergeInProgress() && !s->IsNotFound()) {
        ...
        //              .count(),
        //          ' ', "memtable_num::", cnt);
        // LOG_CERR("early return false");
        return false;
      }
    } else {
      need_remote_read = true;
      assert(memtable->IsTransferCompleted());
      // LOG_CERR("PreGet start");
      // std::chrono::high_resolution_clock::time_point bloom1 =
      //     std::chrono::high_resolution_clock::now();
      // bloom_cnt++;
      // LOG_CERR("cache reads");
      SequenceNumber current_seq = kMaxSequenceNumber;
      bool bloom_contains = false;
      done =
          memtable->CacheGet(key, value, columns, timestamp, s, merge_context,
                        max_covering_tombstone_seq, &current_seq, read_opts,
                        true /* immutable_memtable */, callback, is_blob_index, true, read_client, nullptr, cfd_id, cfd_->index_cache(), &bloom_contains);
      // LOG_CERR("bloom: ", bloom_contains);
      if (*seq == kMaxSequenceNumber) {
        *seq = current_seq;
      }
      if (done) {
      ...
      if (bloom_contains) {
          mixed_ids.emplace_back(memtable->GetID());
        // std::chrono::time_point<std::chrono::system_clock> now_end =
        //     std::chrono::system_clock::now();
        // LOG_CERR("local_read_all_time::notfound::",
        //          std::chrono::duration_cast<std::chrono::microseconds>(
        ...
        //              .count(),
        //          ' ', "memtable_num::", cnt);
        // enable_multi_one_side_reads = true;
        // return false;
      }
      ...
      // if (prev) {
      //   mixed_ids.emplace_back(memtable->GetID());
      //   if (need_remote_read && enable_local_cache_reads) {
      //     if (cfd_->index_cache()->containsMem(memtable->GetID())) {
      //       IndexCache* cache = cfd_->index_cache();
      //       std::string key_str(k.memtable_key().data());
      //       if (auto entry = cache->getMem(memtable->GetID())->getEntry(key_str)) {
      //         int64_t remote_offset = entry->getOffset();
      //         entry->setFreq(entry->getFreq() + 1);

      //         int64_t local_value_offset = read_client->rdma_mem_.allocate(128);
      //         char* value_offset = read_client->get_buf() + local_value_offset;
      //         auto conn = cfd_->get_cflevel_read_connection();
      //         read_client->rdma_read(conn, 128, local_value_offset, remote_offset);
      //         ASSERT_RW(read_client->poll_completion(conn) == 0);
      //         value->assign(*value_offset,128);
      //       }
    ...
    if (need_remote_read && enable_multi_one_side_reads) {
      // LOG_CERR("multi reads")
      SequenceNumber current_seq = kMaxSequenceNumber;
      done =
          memtable->OneSideGet(key, value, columns, timestamp, s, merge_context,
                        max_covering_tombstone_seq, &current_seq, read_opts,
                        true /* immutable_memtable */, callback, is_blob_index, true, read_client, nullptr, cfd_id);
      if (*seq == kMaxSequenceNumber) {
        *seq = current_seq;
      }
      if (done) {
    ...
    // LOG_CERR("multi reads end")
  }
  // LOG_CERR("for loop end");
  if (need_remote_read && mixed_ids.size() > 0 && false) {
    // std::string now;
    // for (auto id_ : mixed_ids) {
    //   now += std::to_string(id_) + ",";
    // }
    ...
    // LOG_CERR("key_memtable_key pointer: ", key.memtable_key().data());
    // LOG_CERR("key_memtable_key_size: ", key.memtable_key().size());
    // LOG_CERR("value_memtable: ", *value);

    std::memcpy(req_packet->key, key.memtable_key().data(),
                key.memtable_key().size());

    // LOG_CERR("in req key: ", req_packet->key);
    ...
    bool ret = read_client->client_send_request_for_memtable_read_v2(
        conn, req_packet, value, timestamp, cfd_->index_cache());
  ...
  //            ' ', "memtable_num::", cnt);
  // }
  std::chrono::high_resolution_clock::time_point end_end =
      std::chrono::high_resolution_clock::now();
  // LOG_CERR("end to end latency: ", std::chrono::duration_cast<std::chrono::microseconds>(end_end - end_start).count());
