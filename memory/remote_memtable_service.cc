...
void RemoteMemTable::rebuild_remote_memTable(
  ...
  // LOG_CERR("skip_list_ptr_: ", skip_list_ptr_);
  idx_ptr += sizeof(void*);
  meta_begin_ptr_ = *reinterpret_cast<void**>(idx_ptr);
  // LOG_CERR("meta_begin_ptr_: ", meta_begin_ptr_);
  idx_ptr += sizeof(void*);
  for (int i = 0; i < 4 /*sep*/; i++) {
    data_begin_ptr_[i] = *reinterpret_cast<void**>(idx_ptr);
    // LOG_CERR("data_begin_ptr_: ", meta_begin_ptr_);
    idx_ptr += sizeof(void*);
  }
  ...
  LOG_CERR("mem_meta: ", mem_meta);
  rmt_rep->set_head_offset(head_offset_);
  LOG_CERR("buf-local:", reinterpret_cast<char*>(data_begin_ptr_[0]) - reinterpret_cast<char*>(rdma_buf));
  LOG_CERR("buf-remote:", reinterpret_cast<char*>(mem_data[0].first) - reinterpret_cast<char*>(rdma_buf));
  for (int i = 0; i < 4 /*sep*/; i++) {
    rmt_rep->set_shard_local_begin(i, data_begin_ptr_[i]);
    LOG_CERR("shard_local_begin: ", data_begin_ptr_[i]);
    rmt_rep->set_shard_remote_begin(i, mem_data[i].first);
    LOG_CERR("shard_remote_begin: ", mem_data[i].first);
  }
...
void RemoteMemTable::remote_get_v2(void* req_data_v2, void* ret_data, void* rdma_buf) {
  ...
  std::chrono::high_resolution_clock::time_point end =
          std::chrono::high_resolution_clock::now();
  // LOG_CERR("RGet time: ", std::chrono::duration_cast<std::chrono::microseconds>(end - start).count());
